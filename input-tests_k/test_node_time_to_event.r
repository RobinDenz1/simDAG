...
test_that("correct nrow, ncol", {
  ...
  expect_equal(colnames(out), c("sickness_event", "sickness_time", "sickness_past_event_times"))
})
...
test_that("save_past_events working", {
  ...
  expect_true(typeof(out1[sickness_event == TRUE]$sickness_past_event_time) == "character")
  expect_true(is.na(unique(out2[sickness_event == TRUE]$sickness_past_event_time)))
})
