...
Because the `sim_from_dag` function uses the method described above, it requires information about the causal structure and the exact form of the relationship between child nodes and their parents. All of this information has to included in the `root_nodes` and `child_nodes` arguments. This can be done manually (which is the usual strategy when conducting simulation studies) or (partially) using existing data (which may be useful when the interest is in getting a toy dataset resembling real data as closely as possible).
...
For example, lets assume that the data we just generated (`sim_dat`) was our dataset of interest. Let us also assume that we know the true underlying causal diagram and have a rough idea about the nature of the relationship between the nodes (e.g. we know or can reasonably guess the node type). Now all we have to do is create a node lists in accordance to these assumptions first:

```{r}
empty_nodes <- list(list(parents=NULL,
                         type="gaussian",
                         name="age"),
                    list(parents=NULL,
                         type="binomial",
...
This looks a lot like the code used above, except that we are not explicitly defining the actual beta coefficients. We only define the causal structure and the node types. Now we can call the `nodes_from_data` function:

```{r}
nodes <- nodes_from_data(sim_dat, nodes=empty_nodes)
```

It returns an object that includes both a completely defined list of the `root_nodes` and the `child_nodes`, which can be used directly in the `sim_from_dag` function:

```{r}
sim_dat2 <- sim_from_dag(n_sim=10000, root_nodes=nodes$root_nodes,
                         child_nodes=nodes$child_nodes)
```

The `nodes_from_data` function essentially just fits the corresponding models one by one for each node and extracts the relevant data from the models to fill in the gaps in the empty node lists. If we set `return_models` to `TRUE` in the `nodes_from_data` function call above, we can actually see that it used the exact same models we fit earlier to check if the simulation was valid.


# Time-varying covariates

Most real datasets include time-varying covariates, e.g. variables that are measured at multiple points in time that are subject to changes. It is possible to generate this type of data using the `sim_from_dag` function as well. All we need to do is to define an appropriate DAG that directly specifies how the variables change over time. For example, we can extend the simple DAG from above to include a dimension of time:
...
child_nodes <- list(list(parents=c("sex", "age"),
                         ...
                         intercept=12,
                         error=2),
                    list(parents=c("age", "bmi_t1"),
                         type="binomial",
                         name="death_t1",
                         betas=c(0.1, 0.3),
                         intercept=-15),
                    list(parents=c("bmi_t1"),
                         type="gaussian",
                         name="bmi_t2",
                         betas=c(1.1),
                         intercept=0,
                         error=2),
                    list(parents=c("age", "bmi_t2"),
                         type="binomial",
                         name="death_t2",
                         betas=c(0.1, 0.3),
                         intercept=-15))
