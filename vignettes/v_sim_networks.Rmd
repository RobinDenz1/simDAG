...
# Using multiple networks in the same DAG
...
Suppose that the first network is a "friendship network", showing who is friends with who. It looks like this:

```{r, fig.width=6, fig.height=6}
plot(g1)
```

Suppose the second network is a "professional network", showing work relationshipts. This second network looks like this:
...
# Using static networks in discrete-time simulation

It is of course also possible to use the outlined networks-simulation-based approach in discrete-time simulations conducted using the `sim_discrete_time()` function. As a simple example, we will consider the spread of an infectious disease through a network of 18 individuals and 10 distinct points in time.

In this simulation, there is only one variable of interest: the infection status of a person, denoted by `infected` (0 = not infected, 1 = infected). First, we will generate a random graph of 18 individuals with 30 connections between them using the `igraph` package:
...
$$
	...
	\begin{cases}
	  0.05 & \text{if} \quad t = 1 \\
		0.9 & \text{if} \quad t > 1 \text{ and } n_{neighbors} \in (1,2) \\
		0.4 & \text{if} \quad t > 1 \text{ and } n_{neighbors} > 3 \\
		0 & \text{otherwise}
	\end{cases}.
$$

Additionally, once infected, a person stays infected. We can simulate this kind of date using the following code:
...
```{r}
sim <- sim_discrete_time(dag, n_sim=18, max_t=10, save_states="all")
...
for (i in seq_len(10)) {
  ...
  set.seed(124)
  plot(g2, vertex.label.cex=0.8, vertex.label.color="black", size=1,
       main=paste0("Network at t = ", i),
       layout=layout_nicely(g2))
}
```

The infection started with only individual 14 being infected (the index case). From there it spread immediately to individuals 2 and 8 and continued from there. Note that despite the fact that this is a discrete-time simulation, the network structure itself does not change here, because we only used a `network()` call defining a static network. Below we extend this example to the case of dynamic network that additionally changes over time.

# Using dynamic networks in discrete-time simulation
...
sim <- sim_discrete_time(dag, n_sim=18, max_t=5, save_states="all",
                         save_networks=TRUE)
data <- sim2data(sim, to="long")
print(data)
```
...
for (i in seq_len(5)) {
  ...
  set.seed(124)
  plot(g_i, vertex.label.cex=0.8, vertex.label.color="black", size=1,
       main=paste0("Network at t = ", i),
