...
By time-dependent nodes we mean nodes that are created using the `node_td()` function. Currently, the `sim_discrete_event()` function only supports nodes of type `"next_time"`, so the following text is exclusively about time-dependent nodes in `sim_discrete_time()`. In general, definining custom time-dependent nodes works in essentially the same way as for simple root nodes or child nodes. The requirements are:
