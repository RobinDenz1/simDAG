...
* In some special cases, the your custom node function may require information from the supplied `dag` as well. If this is the case, just add a named argument called `dag` to your function and the supplied `dag` will be passed to it automatically whenever `sim_from_dag()` or `sim_discrete_time()` are used.
