...
There is a seemingly endless amount of literature describing models for the Covid-19 pandemic and the associated Covid-19 vaccines (see for example ZITATE). Trying to include all relevant aspects would be an unfeasible task. After deliberating on this literature we decided to include only a few key variables. To make this vignette readable we further limited the variables to only include:

* `age`: The age of the individual in years.
* `sex`: The biological sex (male/female) of the individual.
* `calender_time`: General effects of the passage of time.
...
Instead of diving in deep from the start and trying to include all relevant variables at once, it is often better to build a very simplified version first and to start adding more and more variables to it as we continue. The most important variables for us are the `vaccination`, the `covid`-19 infection and the `sickness`. All of these are variables that have a certain probability of occurence at each point in time. Once they occur, they last for some duration (e.g. someone being sick for two weeks or something similar). After the event is over, there is usually some duration where the person is "immune" to receiving the event again. This is a perfect case for using a time-dependent node of type `time_to_event`.
