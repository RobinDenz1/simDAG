...
It is easy to see that all people died over the course of those 50 years by looking at the `death_event` column. The `death_time` column records the time at which each person died. Because `death` is a terminal event and we set `save_past_event=FALSE`, the `death_past_event_times` columns only contains `NA` values and can be ignored.
