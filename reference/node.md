...
- formula:
  ...
  `"ehreg"`, `"poreg"`, `"ypreg"`, `"time_to_event"` and `"next_time"`
  and for any custom node defined by the user. It is also supported for
  nodes of type `"identity"`, but slightly different input is expected
  in that case. See examples and the associated vignette for an in-depth
  explanation.
