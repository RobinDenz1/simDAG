...
- Added the official citation information for the newly published
...
- This update was only done to fix failing tests on CRAN due to an
  update in the `simr` package. Nothing else changed.
...
- Added the
  [`node_polr()`](https://robindenz1.github.io/simDAG/reference/node_polr.md)
  function to allow generation of data from an ordered logistic or
  probit regression.

Enhancements

- Added support for all `rsurv` package based time-to-event nodes in the
  `model` argument of
...
- There was a slight change to
  ...
  which increases performance if `return_probs=TRUE` is used (avoiding a
  needless
  [`rbernoulli()`](https://robindenz1.github.io/simDAG/reference/rbernoulli.md)
  call). The results of simulations with a `DAG` containing a node with
...
- Fixed a bug where using `n_sim=1` in
