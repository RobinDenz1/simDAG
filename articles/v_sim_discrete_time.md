...
Note that we could also directly use the enhanced `formula` interface
for the `death` node here to get equivalent results without having to
...
the `prob_fun` argument. To visualize the resulting DAG, we can use the
...
![](v_sim_discrete_time_files/figure-html/unnamed-chunk-9-1.png)
...
![](v_sim_discrete_time_files/figure-html/unnamed-chunk-12-1.png)
